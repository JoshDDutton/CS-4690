 ...
 */
function loadCourses(): JQuery.jqXHR<void> {
  return $.ajax({
...
function handleCourseChange(): void {
  const selectedCourse = $('#course').val() as string;
 ...
 */
function handleUvuIdInput(event: JQuery.TriggeredEvent): void {
  let value = ($(event.target).val() as string).replace(/\D/g, '');
  ...
  if (value.length === 8) {
    fetchLogs($('#course').val() as string, value);
  } else {
    $('#logsSection').addClass('d-none');
    logsLoaded = false;
    updateAddLogButton();
 ...
 */
function fetchLogs(courseId: string, uvuId: string): JQuery.jqXHR<void> {
  return $.ajax({
    url: `${API_BASE}/logs`,
    method: 'GET',
    data: { courseId, uvuId },
 ...
 */
function displayLogs(logs: Log[], uvuId: string): void {
 ...
 */
function createLogItem(log: Log): JQuery<HTMLElement> {
 ...
 */
function updateAddLogButton(): void {
  const hasText = ($('#logTextarea').val() as string).trim().length > 0;
 ...
 */
function handleFormSubmit(event: JQuery.SubmitEvent): void {
  event.preventDefault();

  const logText = ($('#logTextarea').val() as string).trim();
  if (!logText || !logsLoaded) return;

  const newLog: Log = {
    courseId: $('#course').val() as string,
    uvuId: $('#uvuId').val() as string,
